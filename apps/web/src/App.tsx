...
function App() {
  ...
  const switchRole = (nextRole: UserRole) => {
    ...
    setMessage(nextRole === "owner" ? "사장님 모드로 전환했습니다." : "예약자 모드로 전환했습니다.");
  };
  ...
  const requestJson = async <T,>(
    ...
    if (!response.ok) {
      const error = new ApiRequestError(body.message || "요청에 실패했습니다.", response.status);
  ...
  useEffect(() => {
    ...
    const restoreSession = async () => {
      try {
        ...
        if (!serviceRole) {
          setScreen("auth");
          setMessage("관리자용 웹 화면은 아직 준비 중입니다.");
          return;
        }
        ...
        if (error instanceof ApiRequestError && error.status === 401) {
          ...
          setScreen("auth");
          setMessage("저장된 로그인 정보를 확인하지 못했습니다. API 서버 상태를 확인해주세요.");
        }
      } finally {
        if (!cancelled) {
          setIsRestoringSession(false);
  ...
  const navigate = (nextScreen: AppScreen) => {
    const serviceRole = currentUser && getServiceRoles(currentUser).includes(role) ? role : null;
  ...
  const activeRole = currentUser && getServiceRoles(currentUser).includes(role) ? role : null;

  if (!currentUser || !activeRole) {
    return authScreen;
  }

  const activeScreen = canAccessScreen(screen, activeRole) ? screen : getHomeScreen(activeRole);
  ...
  return (
    <div className="app-frame">
      ...
      <main className="page-shell">
        {activeScreen === "userHome" ? <UserHome onNavigate={navigate} /> : null}
        {activeScreen === "createRequest" ? <CreateRequest onNavigate={navigate} /> : null}
        {activeScreen === "requestWaiting" ? <RequestWaiting onNavigate={navigate} /> : null}
        {activeScreen === "offers" ? <OfferComparison onNavigate={navigate} /> : null}
        {activeScreen === "confirmation" ? <ReservationConfirmation onNavigate={navigate} /> : null}
        {activeScreen === "ownerHome" ? <OwnerHome onNavigate={navigate} /> : null}
        {activeScreen === "ownerRequestDetail" ? <OwnerRequestDetail onNavigate={navigate} /> : null}
        {activeScreen === "createOffer" ? <CreateOffer onNavigate={navigate} /> : null}
        {activeScreen === "myPage" ? <MyPage userLabel={userLabel} onNavigate={navigate} /> : null}
      </main>
