...
export class ReservationsService {
  ...
  async updateMine(
    ...
    const current = await this.findMineRow(
      user,
      id,
    );
    ...
    if (current.status !== "confirmed") {
      throw new ForbiddenException(
        "확정 상태의 예약만 수정할 수 있습니다.",
      );
    }

    if (dto.headCount !== undefined) {
      if (
        typeof dto.headCount !== "number" ||
        !Number.isInteger(dto.headCount) ||
        dto.headCount <= 0
      ) {
        throw new BadRequestException(
          "예약 인원은 1명 이상이어야 합니다.",
  ...
  async confirmForOwner(
    user: RequestUser,
    id: string,
  ) {
    const current =
      await this.findOwnerReservationRow(
        user,
        id,
      );
  ...
  private async findOwnerReservationRow(
    user: RequestUser,
    id: string,
  ) {
    if (user.role !== "OWNER") {
